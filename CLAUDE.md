...
- **현재 상태 (2026-05-22)**: 조작 개편 검증 통과. 2026-05-21 세션 = ① `EnemyAnimator` 적 애니 인프라 ② **종이 패널 UI**(Z/X/C, Elyqara 2D 이식 — 슬라이드+종이 애니, 검증 완료) 둘 다 ✅. 바간테 딥서치 → 결정: 아이템 안 건드림 / 빌드 정체성 = 스킬+캐릭터. **스킬트리 = 바간테식 어피니티 (2026-05-22 사용자 비전)**: 카드/랜덤 폐기. 캐릭터별 스킬트리 3개 + 스탯창(X 종이 패널). M11 = 런 레이어만(메타 M12+). 88퍽 방화벽 = 노드 독립·한 줄·시너지 금지. 스탯 = 우리 메커닉서 재정의 / 포인트 = 적 처치 XP 레벨업 / Kiyan 3트리 = 검술·방어술·리더십 (5-22 직인). 트리 노드 상세 = 사용자가 '나중에'로 파킹. **M11까지 4단계 실행 로드맵 락 (2026-05-22 직인)**: ① 캐릭터·적 인터넷 에셋 → ② 맵(Meshy+Dungeon Modular Pack) → ③ UI/UX + M11 기능(스킬트리 런 레이어·한 런·HUD) → ④ 전투 폴리싱 + 🎯친구 4명 검증. 숙련도 등 메타 = 검증 통과 후 M12+. 캐릭터도 인터넷 에셋(맥북에어 3D 제작 제약). **다음 = Phase 1**. 상세 → `memory/{roadmap_phases_20260522, skilltree_design_20260522, asset_workflow_lock_20260520, paper_panel_ui_20260521}.md`
...
- **적** ✅: `Elyqara.Enemies` asmdef. `IEnemy` + `EnemyData : ScriptableObject` (HP/속도/어그로/4-phase attack/stopping distance/콘 hitbox/deathAnimSeconds) + `EnemyController : NetworkBehaviour` **Souls-like 6-state FSM** (Idle/Chase/Anticipation/Active/Recovery/Dead) + Transition() 단일 진입점 + 4-phase attack + Stopping Distance + 콘 OverlapSphere hitbox + `EnemyAnimator : NetworkBehaviour` (PlayerAnimator 패턴 미러 — MoveState float 동기 + Attack/Hit/Death 트리거 ClientRpc. 사망 시 deathAnimSeconds 만큼 despawn 지연. 모델 없으면 무동작 — `memory/enemy_animation_infra_20260521.md`). 첫 적 = `Wisp.asset`. `EnemySpawner` 호스트 시작 시 1마리 (Phase 5+ StartRoom prefab 자식)
