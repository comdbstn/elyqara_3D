...
- **MCP 도구 함정**: Unity Editor 재시작 후 Claude session 의 deferred tool list 갱신 안 됨 (캐시). 새 세션 시작 시까지 MCP 도구 schema 못 잡음. 이때 `Edit` + `Bash` 우회
...
- [ ] 표준 패턴 진입 전 외부 자료 (인터넷/깃) 검색했나? (★) — 자체 진단으로 시간 폭발 방지
