...
## 2026-05-20 게임 구조 변경 — 끝없는 던전 / 10층 / 5층 단위 테마

- **사용자 직인**: *"이 던전은 설정상 끝도없이 이어진 던전 ... 던만추의 던전처럼 ... 실제로는 100층이고 더 있는데 ... 우선 10층까지 ... 1층부터 5층까지는 동굴테마 ... 6층부터는 수중동굴이라던가 다른 테마"*
- **변경**: 옛 구조 (3층 × 스테이지 3개 = 9 스테이지) → **끝없는 던전, 이번 구현 = 10층, 한 런 = 1~10층**
- 층 1개 = 던전 1개 = 스테이지 1개 (옛날 = 층 1개 = 스테이지 3개. 1:1 로 단순화)
- **테마 5층 단위**: 1~5층 동굴 / 6~10층 다른 테마. 테마 = `DungeonGenerationData` SO 분리 (코드 X, 데이터)
- 출처 = 사용자 selfesteem 폴더 비전 작업에서 도출
- **미정**: 보스 위치 / "층" vs "스테이지" 용어 통일 / 6~10층 구체 테마
